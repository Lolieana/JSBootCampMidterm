...
const { betterThanAverage } = require('./betterThanAverage');
...
describe('betterThanAverage', () => {
  ...
  it('returns a string', () => {
    const returnedValue = betterThanAverage(
      [60, 70, 75, 90, 100],
      90
    );
  ...
  it('returns the correct string if your score is greater than the average.', () => {
    ...
    expect(returnedValue).to.equal(
      'your score 90 is greater than class average 80'
    );
  });
  ...
  it('returns the correct string if your score is less than the average.', () => {
    ...
    expect(returnedValue).to.equal(
      'your score 85 is lower than class average 89'
    );
  });

  it('returns the correct string if your score is equal to the average', () => {
    const returnedValue = betterThanAverage(
      [55, 60, 70, 70, 80, 85, 85, 95],
      75
    );
    expect(returnedValue).to.equal(
      'your score 75 is equal to the class average'
    );
  });
});
